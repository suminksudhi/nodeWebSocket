...
const ENVS = require('./constant');

let clients = [];
const { HEARTBEAT_INTERVAL, SUBSCRIPTION_RESPONSE_DELAY, UNSUBSCRIPTION_RESPONSE_DELAY } = ENVS;
...
const wsServer = (options) =>  {
    ...
    wss.on('connection', (ws, request) => {
        ...
        ws.on('message', async (data) => {
            console.log(`Server: Request Received: ${new Date().toISOString()}: ${data}`);
            const processedData = stringify(await processRequest(data));
            console.log(`Server: Sending Response: ${new Date().toISOString()}: ${processedData}`);
